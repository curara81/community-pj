import type { Catalog, StoneRecord } from "./types";
import { compactCatalogText } from "./catalog";

export interface ConfirmedLibraryItem {
  name: string;
  nameKo?: string;
  category?: string;
  origin?: string;
...
export function pickConfirmedLibrary(records: StoneRecord[]): ConfirmedLibraryItem[] {
  const confirmed = records.filter((r) => r.confirmed);
  // Deduplicate by name (most recent wins)
  const seen = new Set<string>();
  const unique: ConfirmedLibraryItem[] = [];
  for (const r of confirmed) {
    const key = (r.analysis.name || "").toUpperCase();
    if (!key || seen.has(key)) continue;
    seen.add(key);
    unique.push({
      name: r.analysis.name,
      nameKo: r.analysis.nameKo,
      category: r.analysis.category,
      origin: r.analysis.origin,
      description: r.analysis.description?.slice(0, 200),
    });
...
function compactLibraryText(items: ConfirmedLibraryItem[]): string {
  return items
    .map((it) => {
      const parts = [it.name];
      if (it.nameKo) parts.push(`(${it.nameKo})`);
      if (it.category) parts.push(`[${it.category}]`);
      if (it.origin) parts.push(`산지:${it.origin}`);
      return parts.join(" ");
    })
    .join("\n");
}
...
다음 스키마를 정확히 따르세요:
{
  ...
  "usage": {
    "floorIndoor": "good | ok | avoid",
    "floorOutdoor": "good | ok | avoid",
    "wallIndoor": "good | ok | avoid",
    "exterior": "good | ok | avoid",
    "countertop": "good | ok | avoid",
    "wetArea": "good | ok | avoid",
    "notes": "용도와 관련된 주의사항 (한국어 1~2문장, 예: 산성에 약해 주방 카운터탑은 권장 안 함, 동결-융해 반복으로 외부 사용 시 주의 등)"
  },
...
- usage 평가 기준:
  - good: 해당 용도에 적합 (해당 석재의 표준적인 용도)
  - ok: 사용 가능하나 주의 필요 (실링 처리, 정기 관리, 특정 마감/위치 한정 등)
  - avoid: 권장하지 않음 (구조적/내구성/내수성 문제)
  - 일반 가이드 (한국 기후 = 겨울 -10~-20℃, 동결-융해 반복 고려):
    * 대리석: floor/wallIndoor 좋음, wetArea 가능(실링), countertop 가능(산성 주의), exterior/floorOutdoor 사진과 광택 마감에 따라 ok 또는 avoid
    * 화강석: 거의 모든 용도 좋음 (외부 포함)
    * 포셀린/세라믹/Gres Porcellanato (category=ceramic): 흡수율 매우 낮아 모든 용도 좋음. 한국 외장재 트렌드의 대부분이 이 분류 (라임스톤 룩, 트래버틴 룩 등 포함)
    * 천연 라임스톤: 밀도가 높고 흡수율 낮은 종류(예: Jura, Leccese, 일부 포르투갈산)는 ok. 다공성·연질 라임스톤은 한국 외부에서 avoid. 사진의 표면 결과 색이 매우 매끄럽고 치밀하면 ok, 거칠고 큰 화석/공극이 보이면 외부는 avoid. 시공 디테일(실링, 배수, 노출면)에 좌우되므로 notes에 명기
    * 트래버틴: 공극이 있어 외부는 ok지만 채움 처리 필수, floorOutdoor는 동결 취약하니 avoid 또는 ok with notes
    * 사암/슬레이트: 외부 일반적으로 좋음, 미끄럼 주의는 notes에
    * 오닉스: 장식용·반투명, 바닥과 외부 부적합. 카운터탑은 백라이트 장식 외엔 avoid
    * 인조 쿼츠 (engineered_quartz): 카운터탑 우수, 외부는 자외선 변색 avoid
  - 사진에서 마감(폴리시드/혼드/스트럭처드)이 보이면 그것에 맞춰 조정. 외부는 미끄럼·동결 고려해 혼드/소프트 스트럭처드 위주가 ok
- 응답은 반드시 유효한 JSON 객체.`;
...
export function buildUserPrompt(
  ...
  if (imageCount > 1) {
    lines.push(
      `사진은 같은 돌의 ${imageCount}장의 다른 각도/조명입니다. 모든 사진을 종합해서 가장 정확하게 식별해주세요.`
    );
  }
  const originHint = hints?.originHint?.trim();
  const nameHint = hints?.nameHint?.trim();
  if (originHint || nameHint) {
    lines.push("");
    lines.push("=== USER HINTS (사용자가 알려준 단서, 이 정보와 부합하는 후보를 우선 검토) ===");
    if (originHint) {
      lines.push(`예상 산지: ${originHint}`);
    }
    if (nameHint) {
      lines.push(`이름 단서 (제품명 일부 또는 시리즈): ${nameHint}`);
    }
    lines.push(
      "이 단서들이 사진의 시각적 특징과 모순되면 단서를 무시하고 시각적 판단을 우선하세요. 일치하면 1순위로 강하게 반영하세요."
    );
    lines.push("=== END OF HINTS ===");
  }
  ...
  if (library && library.length > 0) {
    lines.push("");
    lines.push("=== USER CONFIRMED LIBRARY ===");
    lines.push(
      "사용자가 과거에 직접 확인한 (정답으로 검증된) 석재 목록입니다. " +
        "사진의 돌이 이 목록 중 하나와 시각적으로 비슷하다면 우선적으로 고려해주세요."
    );
    lines.push(compactLibraryText(library));
